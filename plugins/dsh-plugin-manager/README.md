...
```bash
dsh plugin add --profile web https://github.com/veildawn/dsh-plugins/releases/download/dsh-plugin-manager@v0.3.0/dsh-plugin-manager-0.3.0.tgz
```
...
npm pack
dsh plugin add --profile web ./plugins/dsh-plugin-manager/dsh-plugin-manager-0.3.0.tgz
```
...
测试覆盖：语义化版本比对、Monorepo Release 标签解析与下载地址生成、社区目录归一化与分类提取、安装/卸载来源白名单校验、profile 已安装版本全量扫描与更新标记、lockfile 条目扫描/健康检测/剥离与 RPC 修复、批量更新与卸载任务执行（fake spawn 验证 CLI 参数与并发限制）、异步平滑重启调度、RPC 请求处理（网络已打桩）、专属图标与客户端 Bundle 完整性校验。
