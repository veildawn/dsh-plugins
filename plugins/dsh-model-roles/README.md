...
| `smol` | 自动分类为短小、机械、低风险的任务 | 使用会话当前模型 |
| `slow` | 自动分类为复杂推理、疑难调试、架构、研究或高风险正确性任务 | 使用会话当前模型 |
| `vision` | 图片输入自动委派给一次性识图子代理 | 使用会话当前模型 |
| `plan` | `/plan` 处于开启状态 | 使用会话当前模型 |
| `designer` | 自动分类为 UI、UX、视觉、交互、布局或产品设计 | 使用会话当前模型 |
| `commit` | 自动分类为提交信息生成或提交专用分析 | 使用会话当前模型 |
...
选择 **智选模式** 后，主请求路由优先级为：插件内部运行时角色 → `plan` → `task` 子代理 →
...
输入框不提供角色选择器，也不注册 `/model-role` 命令。用户只需选择一次 **智选模式**，
...
读取和搜索，工具失败时先诊断并尝试安全替代方案。如果一个回合准备结束时，当前回合最新待办仍有
`pending` 或 `in_progress` 项，插件会从最新实质用户任务创建并激活 DSH 原生 Goal。

Goal Round 会在同一会话中持续排入后续工作，直到整体目标经验证完成。已完成的待办不会触发续跑；
暂停或阻塞的既有 Goal 也不会被插件擅自恢复。用户中止、DSH 判定同一阻塞持续至少 3 个 Goal Round，
或达到可配置的最大轮次时停止。默认上限是 32 轮，可在 **设置 → 模型角色 → 持续工作运行时** 修改或关闭。
...
```sh
dsh plugin --profile web add ./dsh-model-roles-0.6.0.tgz
dsh service restart
```
...
插件自带的 Cordis 配置为空映射。安装后会新增 **智选模式** Preset，但不会自动把它设为
默认模式，因此其他 Agent Preset 的原生行为不受影响：
...
打开 **设置 → 模型角色**，为需要的角色选择 Provider、模型和思考档位后保存。再在会话中
选择 **智选模式** 即可启用。角色设置即时用于下一次请求，不需要重启服务。
...
DSH 对话框中的模型选择就是该会话的默认模型，不在模型角色中重复配置。只有当前会话为
**智选模式** 且命中已配置的专用角色时，插件才在请求组装阶段临时覆盖基础路由；未命中或选择
其他 Preset 时完全保留原生选择。修改映射不会改写历史消息，只影响后续模型请求。
...
- 除 `default` 外的九个 OMP 专用角色可以独立选择模型；`default` 始终来自当前会话；`vision` 通过一次性 DSH 子代理触发，`plan`、`task` 使用 DSH 原生会话事实自动触发，
  ...
  通道完成复核。
- `smol`、`slow`、`designer`、`commit` 由轻量模型阅读本回合任务后自动选择。分类不是关键词猜测，
  模型只允许返回五个固定角色之一。
...
真实 Goal Round Driver 续跑、顾问建议回注以及
