...
读取和搜索，工具失败时先诊断并尝试安全替代方案。沙箱升级参数缺少说明时会注入一次纠正上下文，
让 Agent 撤销无依据的升级或携带非空说明重试，而不是因参数错误结束任务。如果一个回合准备结束时，当前回合最新待办仍有
...
```sh
dsh plugin --profile web add ./dsh-model-roles-0.6.1.tgz
dsh service restart
```
...
真实 Goal Round Driver 续跑、跨回合任务保留、空闲停止传播、Goal/子代理继续、沙箱升级参数自恢复、顾问建议回注以及
