...
test('Conversation mobile CSS keeps a bounded scrollport and permanently hides message and composer stats', () => {
  ...
  assert.match(css, /\.pI_x6G_centerCol,[\s\S]*grid-column:2!important/)
  assert.match(css, /\.wSkVaW_root,[^}]*--dsh-chat-content-width:100%!important/)
  assert.match(css, /\.wSkVaW_widthHandle,[^}]*display:none!important/)
...
test('Composer keeps complete mode labels together and wraps the trailing controls when needed', () => {
  ...
  assert.match(css, /\.Sh0Q9G_trigger[^{]*\{[^}]*width:32px!important/)
