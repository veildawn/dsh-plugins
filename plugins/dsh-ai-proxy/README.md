...
```sh
dsh plugin --profile web add ./dsh-ai-proxy-0.2.14.tgz
dsh service restart
```
...
| `defaultReasoningEffort` | `'highest'` | 切换模型时的默认思考档位（默认打开 highest）。`highest` 选该模型最高已知档；`lowest` 使用网关 ladder 第一档；精确档位名优先精确匹配，缺失时落到最近的较低档 |
