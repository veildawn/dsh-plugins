...
| 插件名称 | 目录 | 说明 | 最新独立版本 |
| :--- | :--- | :--- | :--- |
| **`dsh-plugin-manager`** | [`plugins/dsh-plugin-manager`](plugins/dsh-plugin-manager) | 插件管理中心与自有插件更新/卸载管理器（专属拼图图标、移动端全量响应式触控适配、自有插件一键批量更新(N)、社区21分类、静默直接复制指令、异步平滑重启与自动恢复） | [`v0.3.1`](https://github.com/veildawn/dsh-plugins/releases/tag/dsh-plugin-manager@v0.3.1) |
...
| **`dsh-ai-proxy`** | [`plugins/dsh-ai-proxy`](plugins/dsh-ai-proxy) | AI Proxy Service 网关对接与 LLM Provider 插件（支持 Chat/completions、Anthropic messages、Responses 多格式智能匹配、最高推理强度开关、OAuth 2.0 PKCE 认证、Token 刷新与阶梯推理） | [`v0.2.11`](https://github.com/veildawn/dsh-plugins/releases/tag/dsh-ai-proxy@v0.2.11) |
...
# 3. 安装 AI Proxy 网关插件
dsh plugin add --profile web https://github.com/veildawn/dsh-plugins/releases/download/dsh-ai-proxy@v0.2.11/dsh-ai-proxy-0.2.11.tgz
