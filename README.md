...
| **`dsh-prompt-history`** | [`plugins/dsh-prompt-history`](plugins/dsh-prompt-history) | 提示词历史导航与同步器（严格会话绑定防串门、宿主持久化跨端同步、上下键/触控滑动浏览、草稿暂存、输入法与@菜单智能避让） | [`v0.3.0`](https://github.com/veildawn/dsh-plugins/releases/tag/dsh-prompt-history@v0.3.0) |
