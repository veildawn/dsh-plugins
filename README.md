...
| 插件名称 | 目录 | 说明 | 最新独立版本 |
| :--- | :--- | :--- | :--- |
| **`dsh-model-roles`** | [`plugins/dsh-model-roles`](plugins/dsh-model-roles) | 模型角色路由、自动分工、识图子代理、计划模式与顾问复核 (`/advisor`) | `v0.4.11` |
...
dsh plugin add --profile web https://github.com/veildawn/dsh-plugins/releases/download/dsh-model-roles@v0.4.11/dsh-model-roles-0.4.11.tgz
...
# 升级插件后，请先确认没有运行中的会话再重启；直接重启会把活跃回合记为 interrupted
...
./scripts/release.sh dsh-model-roles 0.4.11
