...
| **`dsh-model-roles`** | [`plugins/dsh-model-roles`](plugins/dsh-model-roles) | OMP 完整 10 角色自动路由（一次性识图子代理、计划模式、任务子代理与顾问复核） | `v0.4.4` |
...
# 4. 安装角色模型路由插件
dsh profile --name web plugin add https://github.com/veildawn/dsh-plugins/releases/download/dsh-model-roles@v0.4.4/dsh-model-roles-0.4.4.tgz
